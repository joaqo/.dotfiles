...
Stuff in ~ like .bashrc, .claude/, .codex/, .config, my orchestrator agent, etc.
...
All browser automation uses **cmux browser** - never Chrome MCP, Chrome DevTools, or `open` commands. Use the `cmux-browser` skill and `cmux browser` CLI for everything: opening pages, clicking, typing, screenshots, JS eval, waiting.
...
## Workflow
- Editor: nvim
- Terminal: cmux (use the `cmux` and `cmux-browser` skills for terminal and browser automation)
- Shell: bash
- Python: uv
