...
Don't leave "development narration" comments: notes that explain what the change does, how it differs from before, or that justify the approach to me during the task. They're useful in chat, not in the file — once the task ships, the reader only sees final code, so a comment about "why it's now blank instead of 'Casa'" or "keeps X only for signup" is noise. Ask: would this still make sense to someone reading the file a year from now with no memory of this task? If it only makes sense as a diff explanation, say it in chat and leave it out of the code.
